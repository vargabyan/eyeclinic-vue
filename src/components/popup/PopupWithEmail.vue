...
const onChangeMail = async (e) => {
  ...
  if (await schema.email().isValid(value)) {
    inputStatus.mail.success = true
    inputStatus.mail.error = false
  } else {
    inputStatus.mail.error = true
    inputStatus.mail.success = false
  }
  inputValues.mail = value
  checkButtonStatus()
}
    ...
    <div class="contact-the-patient">
      ...
      <form action="" class="contact-the-patient_form" @submit="onFinish">
        <div class="contact-the-patient_form_items-wrapper">
          ...
          <label class="contact-the-patient_form_item-label"
                 :class="{'contact-the-patient_form_item-label-successful': inputStatus.mail.success}">
            <label class="contact-the-patient_form_description-label" v-show="inputValues.mail">почта?</label>
            <input class="contact-the-patient_form_item" placeholder="Элестронная почта"
                   :class="{'contact-the-patient_form_item-successful': inputStatus.mail.success, 'contact-the-patient_form_item-unsuccessful': inputStatus.mail.error, 'contact-the-patient_form_item-active': inputValues.mail}"
                   @change="onChangeMail">
            {{ inputStatus.mail.error ? 'Текст ошибки' : '' }}
          </label>
        </div>
